                ...
                <div class="p-6 text-gray-900">
                    ...
                    @if ($user->hasUploadedPaymentProof())
                        ...
                        <div class="mb-8 bg-green-50 border border-green-200 rounded-lg p-6">
                            ...
                            <div class="mb-4">
                                <img src="{{ $user->getPaymentProofUrl() }}" alt="Bukti Pembayaran" class="max-w-md rounded-lg shadow">
                            </div>
                            <p class="text-sm text-gray-600 mb-4">Diupload pada: {{ optional($user->payment_proof_uploaded_at)->format('d/m/Y H:i') }}</p>
                            ...
                            <div class="flex flex-wrap gap-3">
                                <a href="{{ route('payment-proof.view') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded">Lihat Detail</a>
                                @if ($user->payment_status !== 'approved')
                                    <form method="POST" action="{{ route('payment-proof.delete') }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded" onclick="return confirm('Hapus bukti pembayaran dan upload ulang?')">Hapus & Upload Ulang</button>
                                    </form>
                                @endif
                            </div>
                        </div>
                    @else
                        ...
                        <div class="mb-10">
                            ...
                            <form method="POST" action="{{ route('payment-proof.store') }}" enctype="multipart/form-data" class="space-y-6">
                                ...
                                <div>
                                    <label for="payment_proof" class="block text-sm font-medium text-gray-700 mb-2">Pilih / Seret & Lepas Bukti Pembayaran</label>

                                    <!-- Dropzone -->
                                    <div id="dropzone" class="flex flex-col items-center justify-center w-full p-6 border-2 border-dashed rounded-lg bg-gray-50 hover:bg-gray-100 transition cursor-pointer border-blue-300">
                                        ...
                                        <p class="mt-2 text-sm text-gray-600">Seret dan lepas gambar di sini, atau <span class="text-blue-600 underline">klik untuk pilih</span></p>
                                    ...
                                    <div id="image-preview" class="mt-4 hidden">
                                        <div class="flex items-start gap-4">
                                            <img id="image-preview-img" src="" alt="Preview" class="max-w-xs rounded-lg shadow">
                                            <div class="space-y-2">
                                                <button type="button" id="remove-file" class="inline-flex items-center bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded">
                                                    Hapus File
                                                </button>
                                                <p class="text-xs text-gray-500">Pastikan informasi pada bukti transfer terlihat jelas.</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-lg">Upload</button>
                                    <a href="{{ route('pending-approval') }}" class="text-gray-600 hover:text-gray-800 underline">Kembali</a>
                                </div>
                            </form>
                        </div>
                    @endif
